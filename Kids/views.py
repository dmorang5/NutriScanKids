...
from .forms import AnalisisForm, HistorialForm
        ...
        if form.is_valid():
            ...
            if resultado:
                ...
                recomendaciones = [
                    ...
                    "Incluir alimentos ricos en hierro y zinc"
                ]
...
""" @login_required
def analisis_multiple(request):
    if request.method == 'POST':
        form = MultipleAnalisisForm(request.POST, request.FILES)
        if form.is_valid():
            ...
            for img_file in imagenes:
                ...
                if resultado:
                    estado = 'normal'
                    if resultado['posible_desnutricion']:
                        estado = 'riesgo' if len(resultado['signos_desnutricion']) == 1 else 'alerta'
                    
                    resultados.append({
                        'nombre': img_file.name,
                        'estado': estado,
                        'signos': resultado['signos_desnutricion']
                    })
            
            return render(request, 'detector_app/analysis_mul.html', {'resultados': resultados})
    else:
        form = MultipleAnalisisForm()
    
    return render(request, 'detector_app/analysis_mul.html', {'form': form})
 """
...
def historial(request):
    historiales = HistorialMedico.objects.filter(usuario=request.user).order_by('-analisis__fecha')
    return render(request, 'detector/historial.html', {'historiales': historiales}) """
...
class HistorialMedicoListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        for item in queryset:
            ...
            historial.append({
                'fecha': item.fecha_seguimiento or analisis.fecha,
                'riesgo': riesgo,
                'color': bg,
                'texto_color': fg,
            })
